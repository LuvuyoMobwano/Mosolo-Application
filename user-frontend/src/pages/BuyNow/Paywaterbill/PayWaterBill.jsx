...
const PayWaterBill = () => {
  ...
  const handlePayWaterBill = () => {
    ...
    setLoading(true);
    axios.post(`http://localhost:5050/transaction/pay-water-bill/${userID}`, waterBillData)
      .then((response) => {
        setLoading(false);
        if (response.status === 200) {
          setPin(response.data.pin); // Set the pin in the components state.
          enqueueSnackbar('Water Bill Payment Successful', { variant: 'success' });
          //navigate('/water-bill-receipt');
        } else {
  ...
  return (
    <div>
      ...
      >
        <h1 style={styles.accounts}>Pay Water Bill</h1>
        ...
        <button className="buttonstyletwo" onClick={handlePayWaterBill} style={styles.buttonstyletwo}>
          Purchase
        </button>
      </div>
    </div>
